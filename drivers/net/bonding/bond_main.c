...
static void bond_add_vlans_on_slave(struct bonding *bond,
		struct net_device *slave_dev)
{
	struct vlan_entry *vlan;
	const struct net_device_ops *slave_ops = slave_dev->netdev_ops;
		...
		struct net_device *slave_dev)
{
	...
	unreg: if ((slave_dev->features & NETIF_F_HW_VLAN_RX)
			&& slave_ops->ndo_vlan_rx_register)
...
static int bond_set_promiscuity(struct bonding *bond, int inc)
{
	...
	if (USES_PRIMARY(bond->params.mode))
	{
		...
		{
			err = dev_set_promiscuity(bond->curr_active_slave->dev, inc);
		}
	}
	else
	{
...
static int bond_set_allmulti(struct bonding *bond, int inc)
{
	...
	if (USES_PRIMARY(bond->params.mode))
	{
		...
		if (bond->curr_active_slave)
		{
			err = dev_set_allmulti(bond->curr_active_slave->dev, inc);
		}
	}
	else
	{
...
{
	struct bonding
	*bond = container_of(work, struct bonding,
			mcast_work.work);
...
static void bond_do_fail_over_mac(struct bonding *bond,
		struct slave *new_active, struct slave *old_active) __releases(&bond->curr_slave_lock)
...
__acquires(&bond->curr_slave_lock)
{
	...
	case BOND_FOM_FOLLOW:
		...
		if (old_active)
		{
			...
			memcpy(saddr.sa_data, old_active->dev->dev_addr, ETH_ALEN);
		...
		if (new_active->link == BOND_LINK_BACK)
		{
			if (USES_PRIMARY(bond->params.mode))
			{
				pr_info(
						...
						(bond->params.updelay - new_active->delay)
								* bond->params.miimon);
			}
...
static u32 bond_fix_features(struct net_device *dev, u32 features)
{
	...
	bond_for_each_slave(bond, slave, i)
	{
		features = netdev_increment_features(features, slave->dev->features,
				mask);
	}
		...
		struct net_device *slave_dev)
{
	...
	memcpy(bond_dev->broadcast, slave_dev->broadcast, slave_dev->addr_len);
...
static bool bond_should_deliver_exact_match(struct sk_buff *skb,
		struct slave *slave, struct bonding *bond)
{
...
int bond_enslave(struct net_device *bond_dev, struct net_device *slave_dev)
{
	...
	if (bond->slave_cnt == 0)
	{
		if (bond_dev->type != slave_dev->type)
		{
			...
			res = netdev_bonding_change(bond_dev, NETDEV_PRE_TYPE_CHANGE);
			...
			netdev_bonding_change(bond_dev, NETDEV_POST_TYPE_CHANGE);
		}
	}
	...
	if (is_zero_ether_addr(bond->dev->dev_addr))
		memcpy(bond->dev->dev_addr, slave_dev->dev_addr, slave_dev->addr_len);
	...
	{
		pr_warning(
				"%s: Warning: failed to get speed and duplex from %s, assumed to be 100Mb/sec and Full.\n",
				bond_dev->name, new_slave->dev->name);

		if (bond->params.mode == BOND_MODE_8023AD)
		{
			pr_warning(
					"%s: Warning: Operation of 802.3ad mode requires ETHTOOL support in base driver for proper aggregator selection.\n",
					bond_dev->name);
		}
	}
	...
	case BOND_MODE_8023AD:
		...
		{
			SLAVE_AD_INFO(new_slave).id = SLAVE_AD_INFO(new_slave->prev).id + 1;
		}
	...
	err_dest_symlinks: bond_destroy_slave_symlinks(bond_dev, slave_dev);

	err_close: dev_close(slave_dev);

	err_unset_master: netdev_set_bond_master(slave_dev, NULL);

	err_restore_mac: if (!bond->params.fail_over_mac)
	{
	...
	err_free: kfree(new_slave);

	err_undo_flags: bond_compute_features(bond);
...
int bond_release(struct net_device *bond_dev, struct net_device *slave_dev)
{
	...
	if (!bond->params.fail_over_mac)
	{
		if (!compare_ether_addr(bond_dev->dev_addr, slave->perm_hwaddr)
			...
			pr_warning(
					...
					bond_dev->name, slave_dev->name, slave->perm_hwaddr,
					bond_dev->name, slave_dev->name);
	}
	...
	pr_info("%s: releasing %s interface %s\n", bond_dev->name,
			bond_is_active_slave(slave) ? "active" : "backup", slave_dev->name);
 ...
 */
static int bond_ioctl_change_active(struct net_device *bond_dev,
		struct net_device *slave_dev)
{
	struct bonding *bond = netdev_priv(bond_dev);
	...
	if ((new_active) && (old_active) && (new_active->link == BOND_LINK_UP)
...
static int bond_slave_info_query(struct net_device *bond_dev,
		struct ifslave *info)
{
	struct bonding *bond = netdev_priv(bond_dev);
	struct slave *slave;
		...
		case BOND_LINK_DOWN:
			...
			if (slave->delay)
			{
				pr_info(
						...
						ignore_updelay ?
								0 : bond->params.updelay * bond->params.miimon);
			}
			/*FALLTHRU*/
		case BOND_LINK_BACK:
			if (!link_state)
	...
	bond_for_each_slave(bond, slave, i)
	{
		...
		case BOND_LINK_UP:
			...
			pr_info(
					...
					bond->dev->name, slave->dev->name, slave->speed,
			...
			if (!bond->curr_active_slave || (slave == bond->primary_slave))
				goto do_failover;
		...
		__be32 dest_ip, __be32 src_ip, unsigned short vlan_id)
{
	...
	skb = arp_create(arp_op, ETH_P_ARP, dest_ip, slave_dev, src_ip, NULL,
			slave_dev->dev_addr, NULL);
...
static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
{
	...
	for (i = 0; (i < BOND_MAX_ARP_TARGETS); i++)
	{
		 ...
		 */
		rt = ip_route_output(dev_net(bond->dev), targets[i], 0, RTO_ONLINK, 0);
		if (IS_ERR(rt))
		{
			if (net_ratelimit())
			{
		...
		{
			ip_rt_put(rt);
			bond_arp_send(slave->dev, ARPOP_REQUEST, targets[i], vlan->vlan_ip,
					vlan_id);
			continue;
		}
...
static void bond_validate_arp(struct bonding *bond, struct slave *slave,
		__be32 sip, __be32 tip)
{
	...
	for (i = 0; (i < BOND_MAX_ARP_TARGETS) && targets[i]; i++)
	{
		pr_debug("bva: sip %pI4 tip %pI4 t[%d] %pI4 bhti(tip) %d\n", &sip, &tip,
				i, &targets[i], bond_has_this_ip(bond, tip));
		if (sip == targets[i])
		{
		...
		struct slave *slave)
{
	...
	arp = arp_hdr(skb);
	if (arp->ar_hln != bond->dev->addr_len || skb->pkt_type == PACKET_OTHERHOST
			|| skb->pkt_type == PACKET_LOOPBACK
			|| arp->ar_hrd != htons(ARPHRD_ETHER)
			|| arp->ar_pro != htons(ETH_P_IP) || arp->ar_pln != 4)
		goto out_unlock;
	...
	pr_debug("bond_arp_rcv: %s %s/%d av %d sv %d sip %pI4 tip %pI4\n",
			...
			bond->params.arp_validate, slave_do_arp_validate(bond, slave), &sip,
			&tip);
 ...
 * dev_probe_lock).
 */
static int bond_netdev_event(struct notifier_block *this, unsigned long event,
		void *ptr)
{
 ...
 */
static int bond_inetaddr_event(struct notifier_block *this, unsigned long event,
		void *ptr)
{
...
static int bond_xmit_hash_policy_l34(struct sk_buff *skb, int count)
{
	...
	if (skb->protocol == htons(ETH_P_IP))
	{
		...
		return (layer4_xor ^ ((ntohl(iph->saddr ^ iph->daddr)) & 0xffff))
				% count;
		...
		struct rtnl_link_stats64 *stats)
{
	...
	bond_for_each_slave(bond, slave, i)
	{
		const struct rtnl_link_stats64 *sstats = dev_get_stats(slave->dev,
				&temp);
...
static int bond_do_ioctl(struct net_device *bond_dev, struct ifreq *ifr,
		int cmd)
{
	...
	case SIOCBONDINFOQUERY:
		...
		if (res == 0 && copy_to_user(u_binfo, &k_binfo, sizeof(ifbond)))
			return -EFAULT;
	...
	case SIOCBONDSLAVEINFOQUERY:
		...
		if (res == 0 && copy_to_user(u_sinfo, &k_sinfo, sizeof(ifslave)))
			return -EFAULT;
...
static bool bond_addr_in_mc_list(unsigned char *addr,
		struct netdev_hw_addr_list *list, int addrlen)
{
	struct netdev_hw_addr *ha;
...
static void bond_set_multicast_list(struct net_device *bond_dev)
{
	...
	__hw_addr_add_multiple(&bond->mc_list, &bond_dev->mc, bond_dev->addr_len,
...
static int bond_set_mac_address(struct net_device *bond_dev, void *addr)
{
	...
	unwind: memcpy(tmp_sa.sa_data, bond_dev->dev_addr, bond_dev->addr_len);
 ...
 */
static int bond_xmit_activebackup(struct sk_buff *skb,
		struct net_device *bond_dev)
{
	struct bonding *bond = netdev_priv(bond_dev);
	int res = 1;
 ...
 */
static inline int bond_slave_override(struct bonding *bond, struct sk_buff *skb)
{
	int i, res = 1;
	...
	/* Find out if any slaves have the same mapping as this skb. */bond_for_each_slave(bond, check_slave, i)
	{
...
static netdev_tx_t __bond_start_xmit(struct sk_buff *skb,
...
{ .get_drvinfo = bond_ethtool_get_drvinfo, .get_link = ethtool_op_get_link, };

static const struct net_device_ops bond_netdev_ops =
{ .ndo_init = bond_init, .ndo_uninit = bond_uninit, .ndo_open = bond_open,
		.ndo_stop = bond_close, .ndo_start_xmit = bond_start_xmit,
		.ndo_select_queue = bond_select_queue,
		.ndo_get_stats64 = bond_get_stats, .ndo_do_ioctl = bond_do_ioctl,
		.ndo_set_multicast_list = bond_set_multicast_list, .ndo_change_mtu =
				bond_change_mtu, .ndo_set_mac_address = bond_set_mac_address,
		.ndo_neigh_setup = bond_neigh_setup, .ndo_vlan_rx_register =
				bond_vlan_rx_register, .ndo_vlan_rx_add_vid =
				bond_vlan_rx_add_vid, .ndo_vlan_rx_kill_vid =
...
#endif
		.ndo_add_slave = bond_enslave, .ndo_del_slave = bond_release,
...
static void bond_setup(struct net_device *bond_dev)
{
	...
	bond_dev->hw_features = BOND_VLAN_FEATURES | NETIF_F_HW_VLAN_TX
...
static int bond_check_params(struct bond_params *params)
{
	...
	if (xmit_hash_policy)
	{
		if ((bond_mode != BOND_MODE_XOR) && (bond_mode != BOND_MODE_8023AD))
		{
		...
		else
		{
			...
			if (xmit_hashtype == -1)
			{
				pr_err("Error: Invalid xmit_hash_policy \"%s\"\n",
						xmit_hash_policy == NULL ? "NULL" : xmit_hash_policy);
				return -EINVAL;
			}
		}
	}
	...
	if (arp_validate)
	{
		...
		arp_validate_value = bond_parse_parm(arp_validate, arp_validate_tbl);
	...
	else if (arp_interval)
	{
		...
		pr_info("ARP monitoring set to %d ms, validate %s, with %d target(s):",
				arp_interval, arp_validate_tbl[arp_validate_value].modename,
				arp_ip_count);
	...
	if (primary && primary_reselect)
	{
		...
		if (primary_reselect_value == -1)
		{
			pr_err("Error: Invalid primary_reselect \"%s\"\n",
					primary_reselect == NULL ? "NULL" : primary_reselect);
			return -EINVAL;
		}
	}
	else
	...
	if (fail_over_mac)
	{
		fail_over_mac_value = bond_parse_parm(fail_over_mac, fail_over_mac_tbl);
...
static void bond_set_lockdep_class_one(struct net_device *dev,
		struct netdev_queue *txq, void *_unused)
{
	lockdep_set_class(&txq->_xmit_lock, &bonding_netdev_xmit_lock_key);
}

static void bond_set_lockdep_class(struct net_device *dev)
{
	lockdep_set_class(&dev->addr_list_lock, &bonding_netdev_addr_lock_key);
...
{ .kind = "bond", .priv_size = sizeof(struct bonding), .setup = bond_setup,
...
int bond_create(struct net *net, const char *name)
{
	...
	bond_dev = alloc_netdev_mq(sizeof(struct bonding), name ? name : "bond%d",
